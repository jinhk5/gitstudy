...
public class SelectProductEtcCommand implements MainCommand {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		ArrayList<ProductDTO> list = cDAO.selectProductEtc1(pCategory);
		...
		list = cDAO.selectProductEtc2(pCategory, beginRecord, endRecord);
		...
		// 다음 페이지로 넘어갈 때 어떤 카테고리인지를 확인하기 위해 pCategory를 모델에 넣어준다. 
		model.addAttribute("pCategory", pCategory);
		// 페이징 model에 넣어주기
		model.addAttribute("pVO", pVO);
		// 어떤 mapping으로 가야할지를 각각 다르게 넣어준다. 
		model.addAttribute("selectList", "selectProductEtc");
	}
